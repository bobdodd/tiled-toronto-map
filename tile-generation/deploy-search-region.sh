...
#   (append by osm_id, no drop) -> DELETE the NDJSON from the server.
#
# The NDJSON is an INTERMEDIATE build file, not kept on the server (disk is scarce).
...
echo "[3/6] decompress on the VPS (transient — removed in step 5)..."
$SSH "$VPS" "zstd -d -q -f ${MAPDATA}.zst -o $MAPDATA && rm -f ${MAPDATA}.zst && echo \"  \$(wc -l < $MAPDATA) docs landed\""

echo "[4/6] upsert into OpenSearch (append by osm_id, no drop)..."
$SSH "$VPS" "cd $SITE && OPENSEARCH_URL=http://localhost:9200 node_modules/.bin/tsx scripts/upsert-map.ts $MAPDATA"

echo "[5/6] remove the NDJSON from the server — it's an intermediate build file; the OpenSearch"
echo "      index is the live copy, and the .pbf extract on the build box is the source-of-truth."
$SSH "$VPS" "rm -f $MAPDATA && echo '  removed from /home/ubuntu/map-data/'"

echo "[6/6] verify — index doc count now:"
